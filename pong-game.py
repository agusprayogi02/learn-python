...
left_paddle = Entity(scale=(1/32, 6/32), model='quad', x=-.75,
                     origin_x=.5, collider='box')
right_paddle = duplicate(left_paddle, x=left_paddle.x*-1,
                         rotate_z=left_paddle.rotation_z+180)

floor = Entity(model='quad', y=-.5, origin_y=.5,
               collider='box', scale=(2, 10), visible=False)
ceiling = duplicate(floor, y=.5, rotation_z=180, visible=False)
left_wall = duplicate(floor, x=-.5*window.aspect_ratio,
                      rotation_z=90, visible=True)
right_wall = duplicate(floor, x=.5*window.aspect_ratio,
                       rotation_z=-90, visible=True)


collision_cooldown = .15
ball = Entity(model='circle', scale=.05, collider='box',
              speed=0, collision_cooldown=collision_cooldown)
...
def update():
    ...
    right_paddle.y += (held_keys['up arrow'] -
                       held_keys['down arrow']) * time.dt * 1
    ...
    if hit_info.hit:
        ...
        if hit_info.entity in (left_paddle, right_paddle, left_wall, right_wall):
            ...
            if hit_info.entity == left_paddle:
                ...
                # disable collision for the current paddle so it doesn't collide twice in a row
            ...
            ball.rotation_z -= (hit_info.entity.world_y -
                                ball.y) * 20 * 32 * direction_multiplier
            ball.speed *= 1.1
        ...
        particle = Entity(model='quad', position=hit_info.world_point,
                          scale=0, texture='circle', add_to_scene_entities=False)
