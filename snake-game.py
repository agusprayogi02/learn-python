...
def message(msgs, color):
    pos = dis_width / 6, dis_height / 3
    for i in range(len(msgs)):
        mesg = font_style.render(msgs[i], True, color)
        dis.blit(mesg, [pos[0], pos[1]+(i*30)])
...
def gameLoop():
    ...
    while not game_over:
        ...
        while game_close:
            dis.fill(blue)
            txt = ["Game Over", "Press Enter To Play Again", "Press Q To Quit"]
            message(txt, red)
